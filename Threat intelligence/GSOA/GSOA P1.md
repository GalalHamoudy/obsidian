...
- **Modern Times:** While betrayals and lies (like the Trojan Horse) have always existed, the **internet and digital media** have changed everything by allowing disinformation to spread at a massive speed and scale.
...
- **The Method:** The information is changed or manipulated so that it **looks like an official piece of news**, but its true goal is to influence the person reading it.
...
- **The Appearance:** Fake news is made to look like a normal, everyday news article. At first glance, it might seem believable, but once an analyst examines it closely, they can see it is a lie.
...
- **Analyzing the Target:** There are two main groups to consider:
    1. **Like-minded:** People who already mostly agree with the story. They just need a small push to help the actor gain more momentum.
    2. **Opposite:** People who disagree with the story. The actor tries to persuade them or use other people to make the division in society even worse.
- The actor's goal is to get the target to **accept or support** the message. If the target ignores it, the message was not crafted well enough.
...
To be a good analyst, you must understand the specific forms disinformation takes so you can turn raw information into useful intelligence. The human brain isn't always on high alert, making it hard to notice when something is staged or manipulated. There are six main forms:
...
- **Deep Fakes:** Does the footage use computer-generated faces or voices?.
- **Wrong Narrative:** Is it a real video but being used to tell a completely different, false story?.
...
- **Online Presence:** Most real people have accounts on multiple platforms. Fake accounts often exist only on one.
- **Friends and Followers:** Many fake accounts follow thousands of people but have very few followers themselves because they don’t have real-world friends.
- **Account Creation Date:** Fake accounts are often created very recently, specifically for a single disinformation campaign.
...
4. **Small Presence:** A Google search for the name only found that one Twitter account, which is unusual for a real person.
...
- **One-Way Traffic:** Real humans interact and have opinions. Fake accounts often just post quotes or links without talking to anyone else.
...
- **When** was it first seen and when was the account made?
- **Why** is it being spread (what is the goal)?
- **How** is it being spread (manually or by bots)?

**Example: The Macron Profile** The text shows two Twitter profiles for French President Emmanuel Macron. By comparing them, you can see one is real and one is fake:

- The fake one is spelled "@Emmanuel**Mascron**" (with an extra 's').
- The real one has a **verified blue checkmark**.
- The real account joined in 2013, while the fake one joined in 2020.
- The real account has millions of followers; the fake one has only 7.

#### **2. CrowdTangle**

This is a tool owned by Meta (Facebook). It helps you see how links are being shared across Facebook, Twitter, Reddit, and Instagram. It is very useful for seeing who is trying to make a specific story go viral.

#### **3. Hoaxy**

Hoaxy visualizes how articles spread on Twitter. It creates a map of "nodes" showing who connected to whom and who spread the message first.

#### **4. Botometer**

This tool uses a computer algorithm to give a Twitter account a score from 0 to 5. A higher score means the account is more likely to be a bot.

#### **5. Reverse Image Search**

This technique helps you find where an image came from. You can crop, flip, or blur parts of the image to find the original source and see if the current context is a lie.
...
**Archiving tools** like the **Wayback Machine (archive.org)** allow you to see what a website looked like in the past. This is vital for OSINT because bad actors often delete their posts or change their stories later.
...
- **EU Factcheck:** Specifically targets disinformation within Europe across many languages.
- **Know Your Meme:** Documents internet memes and viral trends to see how they started and if they are being used for disinformation.
...
OSINT is entirely driven by data, and an investigator will encounter many different types while working. Collecting and analyzing this data efficiently is what makes your research successful, and **using automation is considered a requirement** to handle the large amounts of information.
...
### **Comparing JSON and Delimited Data**

- **JSON Advantages:** It is better for complex records and can handle data where some fields might be missing or different for every person.
- **Delimited (CSV) Advantages:** Most analysts are more familiar with this format because it looks like a spreadsheet. It is also more "compact," meaning the file size is smaller for simple lists.

---

### **A Model for Automating Data Assessment**

When you get a new, large file of data, you need a consistent way to check it. This initial assessment helps you decide if the data is actually relevant to your investigation.

#### **Step 1: Timestamps**

Record exactly when you start and finish your work. This helps maintain a clear sequence of events for your **evidentiary chain**. Using a standard format (ISO 8601) ensures there is no confusion about the date or time zone.

#### **Step 2: Cryptographic Hashes**

Immediately create a "hash" of the file. A hash is like a unique digital fingerprint. If the file is changed even a little bit, the hash will change. This proves that your analysis did not accidentally alter the evidence. The most recommended algorithm to use is **SHA-256**.

#### **Step 3: Determining File Type**

Do not trust the file extension (like .txt or .jpg), as these can be changed or hidden. In Linux, you can use the `file` utility, which looks at the actual data inside the file to tell you what it really is.

#### **Step 4: Record Count**

Instead of just looking at the file size in megabytes, you want to know how many actual records of information are inside. The `wc -l` command counts the lines for you.

#### **Step 5: Identifying Data Fields**

You need to see what kind of information is in the file. Using the `head` utility lets you look at just the first few lines. If there is a header row, it will tell you exactly what each column contains.

#### **Step 6: Extracting Specific Information**

If you only care about one thing (like a list of Instagram IDs), you can use the `cut` utility to extract just that one column from a large file.

#### **Step 7: Unique Values and Keyword Searching**

- **Unique Values:** Large files might have the same information repeated many times. Using `sort -u` lets you see only the unique entries.
- **Histograms:** You can even count how many times each item appears (like seeing which password was used most often) using a combination of `sort` and `uniq -c`.
- **Keywords:** The `grep` command is very powerful for finding specific words (like a specific email domain) inside a huge pile of data.

---

### **The Power of Pipelines**

In the Linux shell, you can use a **pipeline** character `|` to take the output of one tool and immediately feed it into another. For example, you can use `cut` on a column and then immediately `sort` it for unique values, all in one single line of command.

---

### **Limitations and Solutions**

The manual steps described above are useful but can be slow if you have to do them one by one for every new file. Analysts need a way to combine all these steps into a single automated operation.

For those who use Windows, you can enable the **Windows Subsystem for Linux (WSL)**. This allows you to run all these powerful Linux-style utilities directly on your Windows 10 or 11 computer without the overhead of a virtual machine.

### **Summary of the Section**

In this part, we learned that structured data (like CSV and JSON) is perfect for automated analysis. We explored a standard model for checking new data files using shell utilities to keep our evidence reliable and organized. While these manual steps are a great start, the next part of the course will look at even more advanced ways to automate these tasks using Python.
...
### **Processing JSON with the Command Line**

To handle large amounts of data efficiently, analysts use command-line utilities.

#### **1. Basic Shell Utilities**

You can use `head` and `tail` to see the top or bottom of a "pretty" JSON file. However, if the file is "compact" (all one line), these tools don't work well. In that case, you can use **`json_pp`** to "prettify" the data and make it readable.

#### **2. The Power of `jq`**

The **`jq`** utility is a specialized "JSON processor". Think of it like a surgeon’s scalpel—it allows you to cut exactly the piece of data you want out of a huge JSON file.

**Common `jq` Filters:**

- **The Identity Filter (`'.'`):** This is the simplest filter. It takes the input and prints it back to you in a "pretty," colorized format.
- **Object Identifiers:** You use a dot followed by a field name (like `.fname`) to extract only that specific information.
- **Navigating Nested Data:** Often data is "nested" (hidden inside other categories). For example, if you want to find an email address inside a file from **hunter.io**, you might use a path like **`.data.emails[].value`**.
- **The `select()` Function:** This allows you to search for specific records. For example, you can tell `jq` to only show you records where the email address **contains "adam"**.

---

### **Data Format Conversion (JSON to CSV)**

One of the most useful features of `jq` is the ability to transform data. Since many OSINT tools (and Excel) prefer spreadsheets, you can use the **`@csv`** command in `jq` to "flatten" complex JSON data into a simple **CSV format**.

- **The Process:** You first put the specific fields you want into a list `[ ]` and then pipe that list into `@csv`. This creates a perfect column-based file ready for analysis.

---

### **Where to Practice**

The course recommends using **jqplay.org**. This is an online "sandbox" where you can paste a sample of JSON data and practice writing filters to see the results in real-time.

**Important OPSEC Tip:** Never upload sensitive or private investigation data to websites like jqplay.org; only use it for practice with fake or public data.
...
- **Public API List:** A curated list of free, useful APIs (categorized by type, such as "Anti-Malware" or "Books") can be found on a dedicated **GitHub repository**.
